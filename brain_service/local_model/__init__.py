...
def diagnose(user_message: str, context: dict, knowledge: str) -> dict:
    ...
    system_instructions = (
        ...
        "máquina/filamento' vas a recibir machine_max_speed_x/y/z/e (mm/s que los motores "
        "de ESTA impresora pueden alcanzar) y filament_max_volumetric_speed. NUNCA propongas "
        "travel_speed, velocidades de impresión, ni ningún otro valor de velocidad por "
        "encima de esos límites reales — aunque te parezca una velocidad típica o "
        "razonable para otra impresora, si excede el límite de esta máquina no lo "
        "propongas. Si no tenés ese dato para una clave, no asumas un límite: usá un "
        ...
        "de la interfaz. Si el mensaje del usuario pide algo de eso (ej. 'cambiá a la "
        "Ender 3', 'abrí este archivo', 'laminá la pieza'), NO lo ignores ni respondas con "
        "un diagnóstico genérico del preset actual: en diagnosis_text aclará que no podés "
        "hacer esa acción vos y decile en una frase corta dónde hacerlo manualmente (ej. "
        "el selector de impresora/filamento/perfil en la barra lateral izquierda), y dejá "
